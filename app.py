...
class User(BaseModel):
    ...
    def does_like(self, message):
        return (
            Like.select()
            .where((Like.user == self) & (Like.message == message))
            .exists()
        )
...
# add a like button is right here


class Like(BaseModel):
    user = ForeignKeyField(User, backref="likes")
    message = ForeignKeyField(Message, backref="likes")

    class Meta:
        indexes = ((("user", "message"), True),)


# simple utility function to create tables
def create_tables():
    with database:
        database.create_tables([User, Relationship, Message, Like])
...
@app.template_filter("does_like")
def is_like(user, message):
    return user.does_like(message)
...
def create():
    ...
    if request.method == "POST" and request.form["content"]:
        message = Message.create(
            user=user, content=request.form["content"], pub_date=datetime.datetime.now()
        )
